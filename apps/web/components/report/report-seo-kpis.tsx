...
export function ReportSeoKpis({ data, loading }: { data?: SeoMetrics; loading?: boolean }) {
  if (!data || (data as any).error) {
    return <WidgetWrapper title="SEO — KPIs Organic" icon="🔍" loading={loading}><div style={{ padding: 40, textAlign: "center", color: "#94a3b8", fontSize: 14 }}>{(data as any)?.error || "GSC nu este configurat."}</div></WidgetWrapper>
  }

  const posColor = (data.position ?? 0) <= 5 ? "#059669" : (data.position ?? 0) <= 15 ? "#f59e0b" : "#dc2626"

  return (
    <WidgetWrapper title="SEO — KPIs Organic" icon="🔍">
